...
;(function ($) {
  ...
  var counter = function () {
    if ($(document.body).hasClass('counter-scroll')) {
      ...
      // Đảm bảo chỉ chạy sau khi DOM đã tải xong
      $(window).on('load', function () {
        $(window).scroll(function () {
          var $counter = $('.counter')

          // Kiểm tra xem phần tử .counter có tồn tại trong DOM không
          if ($counter.length > 0) {
            var oTop = $counter.offset().top - window.innerHeight

            // Kiểm tra nếu scroll đã đến vị trí phần tử .counter
            if (a == 0 && $(window).scrollTop() > oTop) {
              if ($().countTo) {
                $counter.find('.number').each(function () {
                  ...
                  // Kiểm tra dữ liệu trước khi gọi countTo
                  if (to && speed) {
                    $(this).countTo({
                      to: to,
                      speed: speed,
